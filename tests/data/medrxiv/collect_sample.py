...
API_BASE_URL = "https://api.biorxiv.org"  # Note: medRxiv uses same API base as bioRxiv

# Query: (machine learning OR deep learning) AND (nlp OR natural language processing)
# medRxiv search uses abstract_title field with match-any or match-all flags
...
# Simplified query terms - medRxiv doesn't support complex boolean well
...
def build_search_url(terms: str, page: int = 0) -> str:
    """Build a medRxiv search URL."""
    # URL pattern from old searcher
    # abstract_title:{query} abstract_title_flags:match-all jcode:medrxiv limit_from:YYYY-MM-DD limit_to:YYYY-MM-DD
...
def collect_medrxiv_sample() -> None:
    """Collect sample data from medRxiv using web scraping and API."""
    print("=" * 60)
    print("medRxiv Sample Data Collector (Web Scraping + API)")
    print("=" * 60)
