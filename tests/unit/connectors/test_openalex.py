...
class TestOpenAlexConnectorSearch:
    ...
    def test_http_error_logs_progress_context(self, simple_query, caplog):
        ...
        with (
            patch.object(searcher, "_get", side_effect=requests.RequestException("timeout")),
            patch.object(searcher, "_rate_limit"),
            caplog.at_level(logging.WARNING, logger="findpapers.connectors.openalex"),
        ):
            searcher.search(simple_query)

        assert any("search stopped early" in m for m in caplog.messages)

    def test_empty_results_before_total_logs_warning(self, simple_query, mock_response, caplog):
        """Results running out before the reported total logs a pagination-limit warning."""
        full_page_results = [{"title": f"Paper {i}"} for i in range(200)]
        full_page = mock_response(
            json_data={
                "meta": {"count": 500, "per_page": 200, "next_cursor": "NEXT"},
                "results": full_page_results,
            }
        )
        full_page.raise_for_status = MagicMock()
        empty_page = self._make_next_page_response(mock_response)
