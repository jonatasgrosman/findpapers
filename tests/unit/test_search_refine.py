...
def test_select_skip(monkeypatch, search: Search, paper: Paper):
    with monkeypatch.context() as context:
        context.setattr(
            findpapers.utils.refinement_tool, '_get_select_question_input', lambda: 'Skip')
...
def test_select_no(monkeypatch, search: Search, paper: Paper):
    with monkeypatch.context() as context:
        context.setattr(findpapers.utils.refinement_tool, '_get_select_question_input', lambda: 'No')
...
def test_select_leave(monkeypatch, search: Search, paper: Paper):
    with monkeypatch.context() as context:
        context.setattr(findpapers.utils.refinement_tool, '_get_select_question_input',
                        lambda: 'Other string')
...
def test_select_yes_and_category(monkeypatch, search: Search, paper: Paper):
    with monkeypatch.context() as context:
        context.setattr(
            findpapers.utils.refinement_tool, '_get_select_question_input', lambda: 'Yes')
        context.setattr(
            findpapers.utils.refinement_tool, '_get_category_question_input', lambda x: x[0])
